...
const Blogs = (props) => {
    ...
    useEffect(() => {
        Axios.get('http://192.168.0.103:4000/blog/all').then(res => {
            setdata(res.data)
    ...
    return (
        <View style={{ ...styles.container }}>
            ...
            <FlatList
                ...
                data={data}
                renderItem={({ item }) => <BlogCard source={{ uri: 'http://192.168.0.103:4000/blog/blogimage/' + item._id }} title={item.title} content={item.content} click={() => toggleModal(item._id)} />}
            ...
            <Modal
                ...
                {
                    modalData.map(data1 => {
                        return <BlogContent key={data1._id} source={{ uri: 'http://192.168.0.103:4000/blog/blogimage/' + data1._id }} title={data1.title} content={data1.content} />
                    })
                }
            </Modal>
        </View>
